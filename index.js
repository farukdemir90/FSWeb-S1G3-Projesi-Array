...
function kopyala(otl) {
  const kopyaArr = [];
  for (let i=0; i<otl.length; i++){
  kopyaArr.push(otl[i]);
}
return kopyaArr;
}
...
function dizi25Cesitmi(orjinalTatlar){
 if (orjinalTatlar.length==25){
   return true;
 }else {
   return false;
  }
...
function cesitEkle(xyz, yeniTat){
  const newArr =[...xyz];
...
function indekstekiCesitiGetir(mmm,nnn){
return mmm[nnn];
}
...
Firma, ürün yelpazesine daha fazla lezzet eklemek istediğinden, sadece ilk veya son aromayı
 keyfi olarak çıkarmak yerine, aroma adına göre aromaları kaldırmaları gerektiğini fark ederler.
...
Aşağıdakileri yapmak için ismeGoreCesitCikar işlevini kullanın:
  ...
  Örneğin:  ismeGoreCesitCikar(orijinalTatlar, "Tarçın") çalıştırılmasıyla,  "Tarçın" aroması 
  kaldırılacağından uzunluğu 24 olan bir dizi döndürülür.
...
function ismeGoreFiltrele(arr, yyy){
  const yeniDizi=[];
  for (let i=0; i<arr.length; i++){
   // const index=arr[i].indexOf(yyy);
  if (arr[i].indexOf(yyy)!=-1){
    yeniDizi.push(arr[i]);
  }
  }
  return yeniDizi;
